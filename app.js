...
let lastScanTime = Date.now(); // Safer time tracking
...
function handleUserSend() {
    ...
    if (text !== "") {
        ...
        // Update the mathematical seed based on the letters typed
...
function runScryingPool() {
    ...
    // Scan the static once every 1000 milliseconds (1 second)
    if (now - lastScanTime >= 1000 && !isCooldown) {
        lastScanTime = now;

        // Check a central 16x16 block of pixels for dark density
        ...
        if (darkPixelCount > (256 * 0.88)) {
            ...
            // 10-second silent cooldown before it can pull another word
            setTimeout(() => { isCooldown = false; }, 10000);
        }
    }

    // Refresh the canvas visuals rapidly to keep the static moving
...
startBtn.addEventListener('click', () => {
    ...
    isActive = true;
    lastScanTime = Date.now(); // Reset the timer when started
