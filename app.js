...
const touchPlate = document.getElementById('touch-plate');

// --- STATE VARIABLES ---
let isActive = false;
let cooldown = false; 
let linguisticSeed = 0; // The mathematical echo of the user's last message
...
function handleUserSend() {
    ...
    if (text !== "") {
        ...
        // Convert the user's sentence into a numerical seed based on character codes
...
// --- IDEOMOTOR TOUCH PLATE LOGIC ---
let isTouching = false;
let lastX = 0;
let lastY = 0;
let tremorAccumulator = 0;

// Handles both Mobile Touch and Desktop Mouse
function startTouch(e) {
    if (!isActive) return;
    isTouching = true;
    touchPlate.classList.add('active');
...
function moveTouch(e) {
    if (!isTouching || cooldown) return;
    
    const clientX = e.touches ? e.touches[0].clientX : e.clientX;
    const clientY = e.touches ? e.touches[0].clientY : e.clientY;
    
    // Calculate microscopic muscle tremors (pixel changes)
    let deltaX = Math.abs(clientX - lastX);
    let deltaY = Math.abs(clientY - lastY);
    
    tremorAccumulator += (deltaX + deltaY);
    
    // THRESHOLD: If enough micro-movement is collected, trigger the transmission
    if (tremorAccumulator > 150) {
        cooldown = true;
        
        // Combine the physical movement math with the linguistic seed
        let rawIndexCalc = Math.floor(clientX * clientY) + linguisticSeed + Math.floor(tremorAccumulator);
        let selectedIndex = rawIndexCalc % words.length;
        
        let pulledWord = words[selectedIndex];
        typeToFeed("ENTITY", pulledWord.toUpperCase(), "entity");
        
        // Cooldown forces you to wait before it pulls another word
...
function endTouch() {
    isTouching = false;
    touchPlate.classList.remove('active');
}

// Bind events for mobile and desktop
touchPlate.addEventListener('touchstart', startTouch);
touchPlate.addEventListener('touchmove', moveTouch);
touchPlate.addEventListener('touchend', endTouch);

touchPlate.addEventListener('mousedown', startTouch);
...
startBtn.addEventListener('click', () => {
    ...
    isActive = true;
    printToFeed("SYS", "System online. Ideomotor plate unlocked. Awaiting physical connection.", "system");
});
// --- UI FUNCTIONS ---
function printToFeed(sender, text, cssClass) {
    const li = document.createElement('li');
